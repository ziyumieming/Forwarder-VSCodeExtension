...
- Class summaries use fields, method signatures, existing or generated method summaries, and 1-hop typed relation context. Before generating a full class summary, the backend may create up to three non-recursive relation briefs for the most relevant neighboring classes. Class summaries track own stale and relation-context stale separately; the popover displays distinct badges.
...
- Hovering a graph node with a stored summary shows `summary-popover.js`; nodes without summaries do not trigger LLM requests. Long-pressing function/method nodes requests a function summary; long-pressing class/interface nodes requests a class summary.
