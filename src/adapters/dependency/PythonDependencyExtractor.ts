...
export class PythonDependencyExtractor {
    ...
    private static extractSignatureTypePositions(document: vscode.TextDocument, symbol: vscode.DocumentSymbol): vscode.Position[] {
        ...
        const colonIndex = text.indexOf(':', paramsClose + 1);
        if (colonIndex >= 0) {
            const returnText = text.substring(paramsClose + 1, colonIndex);
            const returnMatch = /->\s*([\s\S]*)/.exec(returnText);
            if (returnMatch) {
                const typeText = returnMatch[1];
                const typeStart = baseOffset + paramsClose + 1 + returnMatch.index + returnMatch[0].indexOf(typeText);
                this.collectIdentifierPositions(document, typeText, typeStart, positions);
            }
        }
    ...
    private static collectIdentifierPositions(
        ...
        const skipWords = new Set([
            'self', 'cls', 'None', 'Any', 'Callable', 'Optional', 'Union', 'List', 'Dict', 'Set',
            'Tuple', 'Sequence', 'Iterable', 'str', 'int', 'bool', 'float', 'bytes', 'list',
            'dict', 'set', 'tuple'
        ]);
